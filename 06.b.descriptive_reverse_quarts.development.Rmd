---
title: "Quartiles of Grants Described: Development"
author: "Andie Creel"
date: "7/21/2021"
output: 
  pdf_document:
...
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(dirname(inputFile), 'Exploratory_Output/descriptive/grant_quarts_development.pdf')) })
---
...
myWorking <- myWorking %>%
  ...
  filter(type != "C") %>% # combination
  filter(type != "A") # acquisition 
...
```
Interpretation: 
In CA, the counties that receive the most number grants per capita in a given decade have **9-24% less POC** than counties that receive the least number of grants per capita.

Quite a few states have a strong trend. 

In AZ, the counties that receive the most number grants per capita in a given decade have **6-8% more people in poverty** than counties that receive the least number of grants per capita.

No major regional or national trends with quantity per cap and poverty. 

In CO, the counties that receive the most number grants per capita in a given decade have **56-72% more rural people** than counties that receive the least number of grants per capita.

There is a major correlation between rural and quantity per cap.
...
```
Interpretation: 
In NV, the counties that receive the most amount of grant funding per capita in a given decade have **5-14% less POC** than counties that receive the least amount of grant funding per capita.
...
```
Interpretation: 
In MD and DE and other, the counties that receive the highest quantity of grants in a given decade have **4-15% less POC** than counties that receive the smallest quantity of grants.
...
For the majority of the country, the counties that receive the highest quantity of grants in a given decade **are less rural** than counties that receive the smallest quantity of grants.
...
```
In VA, the counties that receive the highest amount of grant funding in a given decade have **3-16% less POC** than counties that receive the smallest amount of grant funding.

There are some states with a moderate trend.

In NM, the counties that receive the highest amount of grant funding in a given decade have **1-2% more people in poverty** than counties that receive the lowest amount of grant funding.

No trends

For the majority of the county, the counties that receive the highest amount of grant funding in a given decade have **are less rural** than counties that receive the smallest amount of grant funding.
