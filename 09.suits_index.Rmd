...
myWorking_per_cap <- myWorking %>%
    ...
    # mutate(quantity_per_cap_tot = sum(quantity_per_cap)) %>% # total quantity per cap
...
avg_amount_per_cap_tot <- round(mean(myWorking_per_cap$amount_per_cap_tot),2)
```
...
* LWCF seems disfavors counties with more POC. This matches up with what we saw in the quartile study, and what Katharine Sims has found.
* Less clear story for rural vs urban counties. I think rural status is so linked with population, that it's correlation what how much funding the state gets from the federal program so it's harder to identify the effect her.  
