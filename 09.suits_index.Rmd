...
myResult_state_1 <- data.frame(matrix(ncol=3, nrow = length(state_dfs)))
colnames(myResult_state_1) <- c("state_fips", "Suits_Amount", "Suits_Quantity" )
for (i in 1:length(state_dfs)) {
  myResult_state_1[i, 1] <- state_dfs[[i]][1,1] #state fips
   myResult_state_1[i, 2] <- getSuits(state_dfs[[i]])$Suits_Amount
     myResult_state_1[i, 3] <- getSuits(state_dfs[[i]])$Suits_Quantity
...
state_choropleth(myMap_amount_1, num_colors = 0)  #num_colors = 0 makes it so white is zero
...
# State by State Picture: Quantity
