...
myResult_state_income <- data.frame(matrix(ncol=2, nrow = length(state_dfs)))
colnames(myResult_state_income) <- c("state_fips", "Suits_Amount" )
for (i in 1:length(state_dfs)) {
  myResult_state_income[i, 1] <- state_dfs[[i]][1,1] #state fips
   myResult_state_income[i, 2] <- getSuits(state_dfs[[i]])$Suits_Amount

}

# drop clean up state_fips codes
myResult_state_income <- myResult_state_income %>%
...
myMap_amount_income <- left_join(state.regions, myResult_state_income, by = c("fips.character" = "state_fips")) %>%
...
printSuits(myResult, x_axis = "Income", caption = "Investment per capita is distributed proportionally across counties with varying average median income. ")

ggsave(filename ="Figures/national_median_income.jpeg", width = 11, height = 8)
...
myState_choropleth(myMap_amount_income, num_colors = 0, legend = "Suits Index" ) +  #num_colors = 0 makes it so white is zero
  ggtitle(label = "Suits indexes for income varries state to state despite being proportional nationally") 
  
ggsave(filename ="Figures/state_median_income.jpeg", width = 11, height = 8)
...
ggsave(filename ="Figures/national_POC.jpeg", width = 11, height = 8)
...
ggsave(filename ="Figures/state_poc.jpeg", width = 11, height = 8)
...
ggsave(filename ="Figures/national_rural.jpeg", width = 11, height = 8)
...
myState_choropleth(myMap_amount_rural, num_colors = 0, legend = "Suits Index" ) +  #num_colors = 0 makes it so white is zero
  ...
  # theme(legend.position="bottom")
ggsave(filename ="Figures/state_rural.jpeg", width = 11, height = 8)
...
one <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_med_income_house)
two <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_med_income_house + avg_poc_pct)
three <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_med_income_house + avg_poc_pct + avg_rural_pct)
four <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_poc_pct + avg_rural_pct + avg_med_income_house | state_fips)
...
one <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_poc_pct)
two <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_poc_pct + avg_med_income_house)
three <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_poc_pct + avg_rural_pct + avg_med_income_house)
four <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_poc_pct + avg_rural_pct + avg_med_income_house | state_fips)
...
avg_amount_per_cap_avg <- round(mean(myWorking_per_cap$amount_per_cap_avg, na.rm = TRUE),2)
```
...
one <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_rural_pct)
two <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_rural_pct + avg_med_income_house)
three <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_poc_pct + avg_rural_pct + avg_med_income_house)
four <- feols(data = myWorking_per_cap, amount_per_cap_avg ~  avg_poc_pct + avg_rural_pct + avg_med_income_house | state_fips)
