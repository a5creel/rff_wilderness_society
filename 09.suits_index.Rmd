...
myWorking <- myWorking %>%
  ...
  # mutate(got_grant = sum(got_grant)) %>%  #total quantity of grants a county got in a single year 
  select(-type) %>% # dropping rows for counties that got multiple grants in the same year
  distinct()
...
The **outcomes of interest** is how the total amount of LWCF funds is distributed across demographic characteristics of interest. 
...
myWorking_per_cap <- myWorking %>%
    ...
    mutate(amount_per_cap_cum = sum(amount_per_cap)) %>% # total amount per cap
    # mutate(quantity_per_cap_cum = sum(quantity_per_cap)) %>% # total quantity per cap
    ...
    select(state_fips, fips, amount_per_cap_cum, avg_med_income_house, avg_poc_pct, avg_rural_pct) %>% # set up reduce down to one entry per county
    distinct() # reduce
...
```
This could be driving by states with more urban areas getting more funds, but then within those states their relatively rural counties get more funding (look at state map)
...
five <- feols(data = myWorking_per_cap, amount_per_cap_cum ~  avg_poc_pct + avg_rural_pct + avg_med_income_house | state_fips)
...
# Notes to self 
* put wealth stuff in appendix because it reduced sample size to post 2000s 
...
test <- myWorking %>%
  group_by(fips) %>%
  mutate(amount_total = sum(amount)) %>%
  filter(amount_total == 0)
