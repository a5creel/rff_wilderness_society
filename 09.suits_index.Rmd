...
#gets vector of fips code
myStateFips <- myWorking %>%
  select(state_fips) %>% 
  distinct()
myStateFips <- as.character(myStateFips[["state_fips"]])

#Gets list with a dataframe for each state
df_list <- lapply(myStateFips, getstats)
getstats<- function(st=x){
...
#ATTN: Need to get suits index for each state and assign it to a dataframe with state abbrev to make a choropleth map 
