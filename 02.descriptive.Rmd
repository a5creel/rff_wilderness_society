...
# --- data manipulation to get quantity of grants per quantile--- #
...
I then categorize counties into four quartiles based on the % of the population that is POC. The first quartile categorizes the whitest counties and the fourth quartile categorizes the couties with the most POC. 
...
Four each of the four quartiles, I calculate the percentage received of the total grants given from 1966 until 2018. This metric is a helpful moment for the total quantity of grants given to the different quartiles, but ignores time trends as well as the amount of funding allocated. 
...
state_choropleth(myOne_map,
  title = "% of grants given to first quantile of POC counties",
  legend = "Percent %",
  zoom = continental_us_states) +
  scale_fill_brewer(palette="YlOrBr")+
  ggtitle(label = "% of total grants given to first quantile POC counties", subtitle =  "(whitest counties)") + 
...
state_choropleth(myTwo_map,
  title = "% of grants given to second quantile of POC counties",
  legend = "Percent %",
  zoom = continental_us_states) +
  scale_fill_brewer(palette="YlOrBr") + 
...
state_choropleth(myThree_map,
  title = "% of grants given to third quantile of POC counties",
  legend = "Percent %",
  zoom = continental_us_states) +
  scale_fill_brewer(palette="YlOrBr")+ 
...
state_choropleth(myFour_map,
  title = "% of grants given to fourth quantile of POC counties",
  legend = "Percent %",
  zoom = continental_us_states) +
  scale_fill_brewer(palette="YlOrBr") + 
  labs(fill = "% received by 4th") + 
    ggtitle(label = "% of grants given to fourth quantile of POC counties", subtitle =  "(counties with most POC)") 

# difference between fourth and first quantile 

myDifference_map <- myPOC_mapData %>%
  select(region, one_pct, four_pct) %>%
  mutate(value = round((four_pct - one_pct)/ four_pct * 100)) %>%
  filter(!is.na(region))

state_choropleth(myDifference_map,
  title = "% difference in quantity grants recieved by fourth quartile and first",
  legend = "Percent %",
  zoom = continental_us_states,
  num_colors = 7) +
  scale_fill_brewer(palette="PuOr") + 
  labs(fill = "% diff. between 4th and 1st")
...
state_choropleth(myPerCap_map1,
  title = "Average per capita grants amount given to first quantile of POC counties",
  legend = "Average per cap. $",
  zoom = continental_us_states) +
  scale_fill_brewer(palette="YlOrBr")+
  ggtitle(label = "Average per capita grants amount given to first quantile of POC counties", 
...
state_choropleth(myPerCap_map2,
  title = "Average per capita grants amount given to second quantile of POC counties",
  legend = "Average per cap. $",
  zoom = continental_us_states) +
  scale_fill_brewer(palette="YlOrBr") + 
  labs(fill = "Average per cap. $")
...
state_choropleth(myPerCap_map3,
  title = "Average per capita grants amount given to thrid quantile of POC counties",
  legend = "Average per cap. $",
  zoom = continental_us_states) +
  scale_fill_brewer(palette="YlOrBr") + 
  labs(fill = "Average per cap. $")
...
state_choropleth(myPerCap_map4,
  title = "Average per capita grants amount given to fourth quantile of POC counties",
  legend = "Average per cap. $",
  zoom = continental_us_states) +
  scale_fill_brewer(palette="YlOrBr") + 
  labs(fill = "Average per cap. $")
...
myPerCap_difference_map <- left_join(x = myPerCap_map4, y = myPerCap_map1, by = "region") %>%
  ...
  mutate(value = round((value_4 - value_1)/ value_4 * 100)) %>%
  filter(!is.na(region))
