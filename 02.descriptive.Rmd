...
I then categorize counties in four quartiles based on the % of the population that are POC. The first quartile categorizes the whitest counties and the fourth quartile categorizes the couties with the most POC. 

When calculating quartiles, I group by state and decade. I group by state because the LWCF is administered at the state level, therefore managers are making **intrastate** comparisons when deciding how to allocate funds. I group by decade because the % of the population that are POC changes throughout time. Therefore, a county that had a significant number of POC in the 70s may have experienced demographic changes by the 2010s. Ideally I would group by year rather than decade, but my demographic characteristics come from the decennial census which is collected at the decage level.  


## Percent of Grants Given to Each Quartile 

I calculate the percentage of grants given to counties in the four different quartiles from 1966 until 2018. This metric is a helpful snapshot for the total quantity of grants given to the different quartiles, but ignores time trends as well as the amount of funding allocated. 
...
```{r DATA-MANIPULATION-2, echo=FALSE, message=FALSE}

# --- data manipulation to get amount per person --- #
# 1.) Group by state and POC quartile, 
# 2.) total funding that year  
